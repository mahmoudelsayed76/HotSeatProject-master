...
using SpotOn.Base;
using Models = SpotOn.DAL.Models;
using Cointeco;

namespace Shared.ConsoleApp
{
    class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            try
            {
                CommonBase.InitializeLogging(@"C:\TEMP\Logs\", "Lib.Console");
                // CommonBase.InitializeFileLogging(@"C:\TEMP\Logs\", "Lib.Console");
        ...
        public async Task GetStartedDemoAsync()
        {
            ...
            CosmosHelper.Initialize(EndpointUri, PrimaryKey, databaseId);
#if DEBUG
            CosmosHelper.Instance.TestAutoIncrement();
#endif 

            return ;
            CosmosHelper.Instance.ConnectContainer(containerId, "/LastName");

            this.container = CosmosHelper.Instance[containerId];
        ...
        {
            var userTable = "User";
            CosmosHelper.Instance.ConnectContainer(userTable);

            var u = new Models.User()
            {
                OrgId = "ORG1",
                FriendlyName = $"{name} {Guid.NewGuid()}"
            };
            CosmosHelper.Instance.Insert(userTable, u);
        }
        // </GetStartedDemoAsync>
